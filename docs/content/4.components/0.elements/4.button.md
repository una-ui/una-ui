...
> You can basically use any sizes that you can imagine, there is no limit.

::alert{type="info"}
The `padding` and `font-size` of the button depends on the `btn-size`. If you want to change the `font-size` and `padding` simultaneously, you can always customize it using utility classes.
::
