...
> ⚡ By default, the content of the accordion is not mounted. This means that the content is not rendered until the accordion is opened. This is to avoid unnecessary rendering of the content. If you want to render the content of the accordion when the page loads for SEO purposes, you can use the `mounted` prop.
